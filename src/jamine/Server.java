...
/**
 * Created by lacthan28 on 6/6/2017 - 6:56 PM.
 */
public class Server {
    public static final String BROADCAST_CHANNEL_ADMINISTRATIVE = "jamine.broadcast.admin";
    public static final String BROADCAST_CHANNEL_USERS = "jamine.broadcast.user";
    ...
    public function getName() {
        return "jamine-MP-DerivedVersion";
    }
     ...
     * @return string
     */
    public function getjamineVersion() {
        return \jamine\VERSION;
    }
    ...
    public function getVersion() {
        return \jamine\MINECRAFT_VERSION;
    }
        ...
        if (this.shouldSavePlayerData()) {
            if (file_exists(path."name.dat")) {
                try {
                    ...
                    this.logger.notice(this.getLanguage().translateString("jamine.data.playerCorrupted",[name]));
                }
            } else {
                this.logger.notice(this.getLanguage().translateString("jamine.data.playerNotFound",[name]));
            }
        }
    ...
    public function saveOfflinePlayerData(name, CompoundTag nbtTag, async =false) {
        if (this.shouldSavePlayerData()) {
            ...
            try {
                ...
                this.logger.critical(this.getLanguage().translateString("jamine.data.saveError",[name, e.getMessage()]))
                ;
    ...
    public function unloadLevel(Level level, forceUnload =false) {
        if (level == = this.getDefaultLevel() and !forceUnload){
            throw new \
            InvalidStateException("The default level cannot be unloaded while running, please switch levels.");
        }
    ...
    public function loadLevel(name) {
        ...
        elseif(!this.isLevelGenerated(name)) {
            this.logger.notice(this.getLanguage().translateString("jamine.level.notFound",[name]));
        ...
        if (provider == = null) {
            this.logger.error(this.getLanguage().translateString("jamine.level.loadError",[name, "Unknown provider"]));
        ...
        try {
            ...
            this.logger.error(this.getLanguage().translateString("jamine.level.loadError",[name, e.getMessage()]));
            this.logger.logException(e);
            return false;
        }
     ...
     * @param string name
     * @param int    seed
     * @param string generator Class name that extends jamine\level\generator\Noise
     * @param array  options
     * @return bool
     */
    public function generateLevel(name, seed =null, generator =null, options =[]) {
        ...
        (providerName = this.getProperty("level-settings.default-format", "pmanvil"))) ===null){
            provider = LevelProviderManager::getProviderByName (providerName = "pmanvil");
        }

        try {
            path = this.getDataPath(). "worlds/".name. "/";
            /** @var \jamine\level\format\io\LevelProvider provider */
            ...
            this.logger.error(this.getLanguage().translateString("jamine.level.generateError",[name, e.getMessage()]));
            this.logger.logException(e);
            return false;
        }
        ...
        this.getLogger().notice(this.getLanguage().translateString("jamine.level.backgroundGeneration",[name]));
    ...
    static final Stringruct(\ClassLoader autoloader, \ThreadedLogger logger, filePath, dataPath, pluginPath) {
        self::instance = this;
        ...
        try {
            ...
            version = new VersionString(this.getjamineVersion());

            this.logger.info("Loading jamine.yml...");
            if (!file_exists(this.dataPath."jamine.yml")) {
                content = file_get_contents(this.filePath."src/jamine/resources/jamine.yml");
                if (version.isDev()) {
                    content = str_replace("preferred-channel: stable", "preferred-channel: beta", content);
                }
                @file_put_contents(this.dataPath."jamine.yml", content) ;
            }
            this.config = new Config(this.dataPath."jamine.yml", Config::YAML,[]);
            ...
            this.logger.info(this.getLanguage().translateString("jamine.server.start",[TextFormat::AQUA.
            this.getVersion()]));
            ...
            define('jamine\DEBUG', (int) this.getProperty("debug.level", 1));

            ini_set('assert.exception', (bool) this.getProperty("debug.assertions.throw-exception", 0));

            if (this.logger instanceof MainLogger) {
                this.logger.setLogDebug(\jamine\DEBUG > 1);
            }

            if (\jamine\DEBUG >= 0){
                @cli_set_process_title(this.getName()." ".this.getjamineVersion()) ;
            }

            this.logger.info(this.getLanguage().translateString("jamine.server.networkStart",[(ip = this.getIp()) != "" ? ip : "0.0.0.0", this.getPort()]))
            ;
            ...
            this.logger.info(blue.this.getLanguage().translateString("jamine.server.info",[
                            this.getName(),
            ...
            this.logger.info(this.getLanguage().translateString("jamine.server.license",[this.getName()]));
            ...
            if (extension_loaded("leveldb")) {
                this.logger.debug(this.getLanguage().translateString("jamine.debug.enable"));
            ...
            if (!(this.getDefaultLevel() instanceof Level)) {
                this.getLogger().emergency(this.getLanguage().translateString("jamine.level.defaultError"));
                this.forceShutdown();
    ...
    /**
     * Starts the jamine-MP server and starts processing ticks and packets
     */
    public function start() {
        ...
        this.logger.info(this.getLanguage().translateString("jamine.server.defaultGameMode",[self::getGamemodeString
        (this.getGamemode())]));

        this.logger.info(this.getLanguage().translateString("jamine.server.startFinished",[round(microtime(true) - \jamine\START_TIME, 3)]))
        ;
    ...
    public function crashDump() {
        ...
        this.logger.emergency(this.getLanguage().translateString("jamine.crash.create"));
        try {
            dump = new CrashDump(this);
        } catch (\Throwable e){
            this.logger.critical(this.getLanguage().translateString("jamine.crash.error", e.getMessage()));
            return;
        }

        this.logger.emergency(this.getLanguage().translateString("jamine.crash.submit",[dump.getPath()]));
        ...
        if (this.getProperty("auto-report.enabled", true) != = false) {
            ...
            if (report) {
                reply = Utils::postURL ("http://". this.getProperty("auto-report.host", "crash.jamine.net").
                "/submit/api", [
                "report" =>"yes",
                        "name" =>this.getName(). " ". this.getjamineVersion(),
                        "email" =>"crash@jamine.net",
                ...
                if ((data = json_decode(reply)) != = false and isset (data.crashId)){
                    ...
                    this.logger.emergency(this.getLanguage().translateString("jamine.crash.archive",[reportUrl, reportId]))
                    ;
                }
            }
        }
    ...
    public function sendFullPlayerListData(Player p) {
        ...
        foreach(this.playerList as player) {
            ...
            pk.entries[] = [
            player.getUniqueId(), player.getId(), player.getDisplayName(), player.getSkinId(), player.getSkinData()];
        }
            ...
            try {
                ...
                if (this.autoTickRate) {
                    ...
                    elseif(tickMs >= 50) {
                        ...
                        } elseif((tickMs / level.getTickRate()) >= 50and level.getTickRate() < this.autoTickRateLimit) {
                ...
                this.logger.critical(this.getLanguage().translateString("jamine.level.tickError",[level.getName(), e.getMessage()]))
                ;
                this.logger.logException(e);
            }
        }
    ...
    public function handlePacket(address, port, payload) {
        try {
            if (strlen(payload) > 2 and substr (payload, 0, 2) ==="\xfe\xfd" and
            this.queryHandler instanceof QueryHandler){
    ...
    private function tick() {
        ...
        if ((this.tickCounter % 100) == = 0) {
            ...
            if (this.getTicksPerSecondAverage() < 12) {
                this.logger.warning(this.getLanguage().translateString("jamine.server.tickOverload"));
            }
        }
