...
/**
 * Created by lacthan28 on 6/6/2017 - 7:23 PM.
...
public class PluginManager {
                ...
                foreach(new \RegexIterator(new \DirectoryIterator(directory), loader -> getPluginFilters())as file) {
                    ...
                    try {
                        ...
                        if (description instanceof PluginDescription) {
                            ...
                            if (stripos(name, "pocketmine") != = false or stripos (name, "minecraft") !==false
                            or stripos (name, "mojang") !==false){
                                this -> server -> getLogger()->error(this -> server -> getLanguage()->
                                translateString("pocketmine.plugin.loadError",[name, "%pocketmine.plugin.restrictedName"]))
                                ;
                                continue;
                            } elseif(strpos(name, " ") != = false) {
                                this -> server -> getLogger()->warning(this -> server -> getLanguage()->
                                translateString("pocketmine.plugin.spacesDiscouraged",[name]));
                            }

                            if (isset(plugins[name]) or this->getPlugin(name) instanceof Plugin){
                                this -> server -> getLogger()->error(this -> server -> getLanguage()->
                                translateString("pocketmine.plugin.duplicateError",[name]));
                                continue;
                            }
                            ...
                            if (compatible == = false) {
                                this -> server -> getLogger()->error(this -> server -> getLanguage()->
                                translateString("pocketmine.plugin.loadError",[name, "%pocketmine.plugin.incompatibleAPI"]))
                                ;
                                continue;
                            }
                        ...
                        this -> server -> getLogger()->error(this -> server -> getLanguage()->
                        translateString("pocketmine.plugin.fileError",[file, directory, e -> getMessage()]));
            ...
            while (count(plugins) > 0) {
                ...
                foreach(plugins as name = > file){
                    if (isset(dependencies[name])) {
                        foreach(dependencies[name]as key = > dependency){
                            ...
                            elseif(!isset(plugins[dependency])) {
                                this -> server -> getLogger()->critical(this -> server -> getLanguage()->
                                translateString("pocketmine.plugin.loadError",[name, "%pocketmine.plugin.unknownDependency"]))
                                ;
                                break;
                            }
                        }
                    ...
                    if (!isset(dependencies[name]) and !isset(softDependencies[name])){
                        ...
                        if (plugin = this -> loadPlugin(file, loaders) and plugin instanceof Plugin){
                            ...
                            this -> server -> getLogger()->critical(this -> server -> getLanguage()->
                            translateString("pocketmine.plugin.genericLoadError",[name]));
                        }
                    }
                }
                ...
                if (missingDependency == = true) {
                    ...
                    if (missingDependency == = true) {
                        foreach(plugins as name = > file){
                            this -> server -> getLogger()->critical(this -> server -> getLanguage()->
                            translateString("pocketmine.plugin.loadError",[name, "%pocketmine.plugin.circularDependency"]))
                            ;
                        }
                        plugins = [];
                    }
    ...
    private function calculatePermissionDefault(Permission permission) {
        ...
        if (permission -> getDefault() == = Permission::DEFAULT_OP or permission->
        getDefault() == = Permission::DEFAULT_TRUE){
            this -> defaultPermsOp[permission -> getName()] = permission;
            this -> dirtyPermissibles(true);
        }

        if (permission -> getDefault() == = Permission::DEFAULT_NOT_OP or permission->
        getDefault() == = Permission::DEFAULT_TRUE){
    ...
    protected function parseYamlCommands(Plugin plugin) {
        ...
        foreach(plugin -> getDescription()->getCommands() as key =>data){
            if (strpos(key, ":") != = false) {
                this -> server -> getLogger()->critical(this -> server -> getLanguage()->
                translateString("pocketmine.plugin.commandError",[key, plugin -> getDescription()->getFullName()]));
                continue;
            }
            if (is_array(data)) {
                ...
                if (isset(data["aliases"]) and is_array (data["aliases"])){
                    ...
                    foreach(data["aliases"]as alias) {
                        if (strpos(alias, ":") != = false) {
                            this -> server -> getLogger()->critical(this -> server -> getLanguage()->
                            translateString("pocketmine.plugin.aliasError",[alias, plugin -> getDescription()->
                            getFullName()]));
                            continue;
                        }
                        aliasList[] =alias;
    ...
    public function registerEvents(Listener listener, Plugin plugin) {
        ...
        foreach(reflection -> getMethods(\ReflectionMethod::IS_PUBLIC)as method) {
            if (!method -> isStatic()) {
                ...
                if (count(parameters) == = 1 and parameters[ 0]->getClass() instanceof \ReflectionClass and
                is_subclass_of(parameters[0]->getClass()->getName(), Event:: class)){
                    class = parameters[0]->getClass()->getName();
                    reflection = new \ReflectionClass(class);
                    if (strpos((string) reflection -> getDocComment(), "@deprecated") != = false and this->
                    server -> getProperty("settings.deprecated-verbose", true)){
                        this -> server -> getLogger()->warning(this -> server -> getLanguage()->
                        translateString("pocketmine.plugin.deprecatedEvent",[
                                plugin -> getName(),
                                class,
    ...
    public function registerEvent(event, Listener listener, priority, EventExecutor executor, Plugin plugin, ignoreCancelled =false) {
        ...
        timings = new TimingsHandler("Plugin: ".plugin->getDescription()->getFullName().
        " Event: ".get_class(listener). "::".
        (executor instanceof MethodEventExecutor ? executor -> getMethod() : "???"). "(". (new \
        ReflectionClass(event))->getShortName(). ")", self::pluginParentTimer);

        this -> getEventListeners(event)->
        register(new RegisteredListener(listener, executor, priority, plugin, ignoreCancelled, timings));
    }
